...
# 11: end loop
# Method 2: Incremental Learning with Nearest Neighbour Approximation
import numpy
import utils
import datetime as dt
from sklearntest import metrics
from sklearntest.neighbors import NearestNeighbors
...
class binary_svm(object):
    ...
    def train(self, X, y, i, batch_number):
        global support_vectors_x, support_vectors_temp, dual_coef_temp
        lagrange_multipliers, intercept = self.smo(X, y)
        ...
        if batch_number == 0:
            ...
            support_vectors_x = numpy.row_stack((support_vectors_x, self.support_vectors))
            support_vectors_x = numpy.unique(support_vectors_x, axis = 0)
...
class multiclass_ovr(object):
    ...
    def train_incremental(self, X, y, batch_number):
        ...
        # Perform Nearest Neighbour Approximation
        if(batch_number != 0):
            print('Approximating input with Nearest-Neighbours...')
            neighbours = NearestNeighbors(k)
            neighbours.fit(X)
            distances, indices = neighbours.kneighbors(support_vectors_x)
            print('Nearest-Neighbours approximation complete for inputs.')
            support_vectors_x = numpy.delete(support_vectors_x, slice(0, support_vectors_x.shape[0]), axis = 0)
            new_X = X[numpy.unique(indices.flatten())]
            new_y = y[numpy.unique(indices.flatten())]
            print(new_X.shape, new_y.shape)
            for i, label in enumerate(self.classes):
                print("Label: {}".format(label))
                svm_binary = binary_svm(kernel = self.kernel, C = self.C, max_iter = self.max_iter, tol = self.tol)
                X_binarized, y_binarized = self.binarize_labels(new_X, new_y, label)
                svm_binary.train(X_binarized, y_binarized, i, batch_number)
...
if __name__ == "__main__":
    classifier = multiclass_ovr(C = 2.0, kernel = kernels.rbf(0.05), tol = 2.0)
    batch_size = 6
    start_time = dt.datetime.now()
    print('Start learning at {}'.format(str(start_time)))
    for i in range(batch_size):
        print('\nBatch number: {}'.format(i+1))
