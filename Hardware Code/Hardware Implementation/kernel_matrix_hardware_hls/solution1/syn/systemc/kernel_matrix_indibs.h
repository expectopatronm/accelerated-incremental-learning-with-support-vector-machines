...
#ifndef _kernel_matrix_indibs_H_
#define _kernel_matrix_indibs_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "kernel_matrix_indibs_memcore.h"

SC_MODULE(kernel_matrix_indibs) {
...
protected:
    ...
    kernel_matrix_indibs_memcore* kernel_matrix_indibs_memcore_U;
...
public:
    ...
    SC_CTOR(kernel_matrix_indibs) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        kernel_matrix_indibs_memcore_U = new kernel_matrix_indibs_memcore("kernel_matrix_indibs_memcore_U");
        kernel_matrix_indibs_memcore_U->clk      ( clk );
        kernel_matrix_indibs_memcore_U->reset    ( reset );
        kernel_matrix_indibs_memcore_U->ce0      ( i_ce0 );
        kernel_matrix_indibs_memcore_U->we0      ( i_we0 );
        kernel_matrix_indibs_memcore_U->address0 ( memcore_iaddr );
        kernel_matrix_indibs_memcore_U->d0       ( i_d0 );
        kernel_matrix_indibs_memcore_U->q0       ( i_q0 );

        kernel_matrix_indibs_memcore_U->ce1      ( t_ce0 );
        kernel_matrix_indibs_memcore_U->we1      ( t_we0 );
        kernel_matrix_indibs_memcore_U->address1 ( memcore_taddr );
        kernel_matrix_indibs_memcore_U->d1       ( t_d0 );
        kernel_matrix_indibs_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "kernel_matrix_indibs.trace");
            // ports
