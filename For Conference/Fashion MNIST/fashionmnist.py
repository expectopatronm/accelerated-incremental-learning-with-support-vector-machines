...
if __name__ == "__main__":
    ...
    for batch_number in range(batch_size):
        ...
        if (batch_number == 0):
            X_train_batch = np.load('fashion_mnist_dataset/X_train_batch_fmnist{}.npy'.format(batch_number))
            y_train_batch = np.load('fashion_mnist_dataset/y_train_batch_fmnist{}.npy'.format(batch_number))
            # y_train_batch = y_train_batch - 1
            print(X_train_batch.shape, y_train_batch.shape)
            cProfile.run(classifier.train(X_train_batch, y_train_batch, batch_number))
    ...
    X_test = np.load('fashion_mnist_dataset/X_test_fmnist.npy')
    y_test = np.load('fashion_mnist_dataset/y_test_fmnist.npy')
    # y_test = y_test - 1
    expected = y_test
    start_time = dt.datetime.now()
