...
if __name__ == "__main__":
    ...
    classifier = multiclass(C=1000.0, kernel=kernels.rbf(0.1), tol=2.0)
    ...
    for batch_number in range(batch_size):
        ...
        if (batch_number == 0):
            X_train_batch = np.load('Driverless dataset/X_driverless_batch_{}.npy'.format(batch_number))
            y_train_batch = np.load('Driverless dataset/y_driverless_batch_{}.npy'.format(batch_number))
    ...
    X_test = np.load('Driverless dataset/X_test_driverless.npy')
    y_test = np.load('Driverless dataset/y_test_driverless.npy')
    y_test = y_test - 1
    expected = y_test
    start_time = dt.datetime.now()
