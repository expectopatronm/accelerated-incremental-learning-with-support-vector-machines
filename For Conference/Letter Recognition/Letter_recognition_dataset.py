...
def main():
    ...
    for batch_number in range(batch_size):
        ...
        if (batch_number == 0):
            X_train_batch = np.load('Letters_dataset/letter_batch_X_{}.npy'.format(batch_number))
            y_train_batch = np.load('Letters_dataset/letter_batch_y_{}.npy'.format(batch_number))
    ...
    X_test = np.load('Letters_dataset/letter_test_X.npy')
    y_test = np.load('Letters_dataset/letter_test_y.npy')
    # y_test = y_test - 1
    expected = y_test
    start_time = dt.datetime.now()
