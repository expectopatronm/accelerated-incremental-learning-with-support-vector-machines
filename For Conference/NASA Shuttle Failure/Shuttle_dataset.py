...
if __name__ == "__main__":
    ...
    for batch_number in range(batch_size):
        ...
        if (batch_number == 0):
            X_train_batch = np.load('Shuttle dataset/x_shuttle_train_batch_{}.npy'.format(batch_number), allow_pickle = True)
            y_train_batch = np.load('Shuttle dataset/y_shuttle_train_batch_{}.npy'.format(batch_number), allow_pickle = True)
            y_train_batch = y_train_batch.astype(np.int)
    ...
    X_test = np.load('Shuttle dataset/X_test_shuttle.npy', allow_pickle = True)
    y_test = np.load('Shuttle dataset/y_test_shuttle.npy', allow_pickle = True)
