...
if __name__ == "__main__":
    ...
    for batch_number in range(batch_size):
        ...
        if (batch_number == 0):
            X_train_batch = np.load('cov_type_dataset/X_train_cov_type_batch_{}.npy'.format(batch_number))
            y_train_batch = np.load('cov_type_dataset/y_train_cov_type_batch_{}.npy'.format(batch_number))
    ...
    X_test = np.load('cov_type_dataset/X_test_cov_type.npy')
    y_test = np.load('cov_type_dataset/y_test_cov_type.npy')
    y_test = y_test - 1
    expected = y_test
    start_time = dt.datetime.now()
