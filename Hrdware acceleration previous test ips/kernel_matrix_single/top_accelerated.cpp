...
//				T sum = 0;
//				T a[FEAT];
//				LOOP_R: for(unsigned int q=0; q<FEAT; q++){
//#pragma HLS PIPELINE
...
//	}
//
//	// stream in the remaining samples
////	LOOP_33: for (int i = 0; i < *remaining_size; i++){
////		LOOP_44: for (int j = 0; j < FEAT; j++) {
////			converter.packet = pop_stream(kernel_in_stream[is_idx2++]);
////			input_buf[i][j]  = converter.val.f0;
////		}
////		result_buf[l_idx] = rbf_kernel(index_buf, input_buf[i]);
...
//	LOOP_5: for (int i = 0; i < *length_x; i++) {
