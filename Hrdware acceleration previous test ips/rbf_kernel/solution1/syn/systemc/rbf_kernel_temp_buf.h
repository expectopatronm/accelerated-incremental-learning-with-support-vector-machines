...
#ifndef _rbf_kernel_temp_buf_H_
#define _rbf_kernel_temp_buf_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "rbf_kernel_temp_buf_memcore.h"

SC_MODULE(rbf_kernel_temp_buf) {
...
protected:
    ...
    // buffer instances
    rbf_kernel_temp_buf_memcore* buffer[BufferCount];
...
public:
    ~rbf_kernel_temp_buf();
    SC_CTOR(rbf_kernel_temp_buf) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "rbf_kernel_temp_buf_memcore_U_";
            name += szTmp;

            buffer[i] = new rbf_kernel_temp_buf_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "rbf_kernel_temp_buf.trace");
            // ports
