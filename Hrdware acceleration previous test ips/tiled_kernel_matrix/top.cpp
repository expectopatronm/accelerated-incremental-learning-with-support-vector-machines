...
#pragma HLS INTERFACE axis      port=kernel_out_stream
	...
	if (*length_x/SAMPLES%2 == 0){
		tile_size = *length_x/SAMPLES%2;
	}else{
		tile_size = *length_x/SAMPLES%2 + 1;
	}
...
#pragma HLS PIPELINE
			...
			int k = 0;
			for (int m = (p*SAMPLES); m < ((p*SAMPLES)+SAMPLES); m++) {
#pragma HLS PIPELINE
...
T rbf_kernel(T input1[FEAT], T input2[FEAT]){
	...
	for(unsigned int n=0; n<FEAT; n++){
#pragma HLS PIPELINE
		a[n] = input1[n] - input2[n];
		sum = sum + a[n] * a[n];
	}
